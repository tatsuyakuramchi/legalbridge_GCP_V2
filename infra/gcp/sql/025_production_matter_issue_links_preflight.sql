...
-- 025_production_matter_issue_links_preflight.sql
-- 読取専用の事前確認（本番）。変更なし（GRANT は 025_production_matter_issue_links_grants.sql）。
...
BEGIN
  ...
  IF to_regclass('public.matter_issues') IS NULL THEN
    RAISE EXCEPTION 'Relation public.matter_issues is missing';
  END IF;
END
$guard$;

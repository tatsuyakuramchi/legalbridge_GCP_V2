...
-- 022_cloudsign_request_history_preflight.sql
-- 読取専用の事前確認。変更は行わない（作成/GRANT は 022_cloudsign_request_history_validation.sql）。
-- 隔離検証DBで、依頼台帳の存在と writer の SELECT/INSERT/UPDATE 権限を確認する。
...
BEGIN
  ...
  IF to_regclass('public.lb_v2_cloudsign_requests') IS NULL THEN
    RAISE EXCEPTION 'Relation public.lb_v2_cloudsign_requests is missing (run 022_cloudsign_request_history_validation.sql first)';
  END IF;
END
$guard$;

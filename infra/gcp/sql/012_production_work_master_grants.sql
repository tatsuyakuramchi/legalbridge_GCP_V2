...
-- 012_production_work_master_grants.sql
-- 作品(works)マスタの編集を legalbridge_v2_runtime に許可する最小権限付与。
--   007 で works の INSERT と works_id_seq は付与済み。ここで UPDATE のみ追加する
--   （DELETE は付与しない）。非破壊・追加のみ。

\if :{?confirm_work_master_grants}
\else
  \echo 'Missing confirmation variable.'
  \echo 'Run with: -v confirm_work_master_grants=GRANT_PRODUCTION_WORK_MASTER'
  \quit 2
\endif

SELECT :'confirm_work_master_grants' = 'GRANT_PRODUCTION_WORK_MASTER' AS confirmed
\gset
...
BEGIN
  ...
  IF to_regclass('public.works') IS NULL THEN
    RAISE EXCEPTION 'Relation public.works is missing';
  END IF;
END
$guard$;
...
-- INSERT と works_id_seq は 007 で付与済み。編集用に UPDATE を追加。

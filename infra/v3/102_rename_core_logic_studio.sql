...
--      ★ (false) を (true) に書き換えてから実行。
--
--      ※ 本番では約50件が当たる（2026-09-11）。そのほとんどは
--        「作品に、作品名と同じ名前のコアロジックが1つ」という標準形で、
--        すでに part_type='game_design' が入っている。V3 は役割を part_type から
--        決めるので、名前を変えても判定は変わらない。名前が長くなるだけ
--        （神我狩 ストーリー＆データ集 混沌神話との接触_Original_Core_Logic）。
--        名前でしか判定できないのは part_type が game_design でない素材で、
--        それは【2】で片づく。流さなくてよい。
-- ---------------------------------------------------------------------
WITH go(ok) AS (VALUES (false)),          -- ★ ここを (true) に
renamed AS (

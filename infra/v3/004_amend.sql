...
\echo '--- 一括作成の束（A-016。表があり、documents.batch_id がある） ---'
SELECT (SELECT string_agg(privilege_type, ', ' ORDER BY privilege_type)
          FROM information_schema.role_table_grants
         WHERE grantee = 'legalbridge_v3_runtime' AND table_name = 'document_batches') AS 権限,
       (SELECT data_type FROM information_schema.columns
         WHERE table_schema = 'v3' AND table_name = 'documents' AND column_name = 'batch_id') AS batch_id;

\echo '--- 条件明細の仕様と帰属先（A-017。2 列であること） ---'
SELECT count(*) AS 列数 FROM information_schema.columns
 WHERE table_schema = 'v3' AND table_name = 'conditions'
   AND column_name IN ('spec', 'deliverable_ownership');

\echo '--- 実績の検収書向けの列（A-018。4 列であること） ---'
SELECT count(*) AS 列数 FROM information_schema.columns
...
\echo '--- 条件の外部の発注番号（A-019。1 列であること） ---'
SELECT count(*) AS 列数 FROM information_schema.columns
 WHERE table_schema = 'v3' AND table_name = 'conditions' AND column_name = 'order_no';

\echo '--- 計算書の一意制約（A-020。document_id 単独の UNIQUE が消え、条件との組になる） ---'
SELECT
  (SELECT count(*) FROM pg_constraint
    WHERE conrelid = 'v3.statements'::regclass
      AND conname = 'statements_document_id_key')          AS 旧制約,
...
\echo '--- 実績の予定との差分・次のアクション（A-030。5 列であること） ---'
SELECT count(*) AS 列数 FROM information_schema.columns
 WHERE table_schema='v3' AND table_name='condition_events'
...
\echo '--- 取引先の代表者と連絡先の役割の印（A-032。3 列であること） ---'
SELECT (SELECT count(*) FROM information_schema.columns
...
\echo '--- 条件ごとの自動更新（A-039。列が 3 つあること） ---'
SELECT count(*) AS 更新の列
  FROM information_schema.columns
 WHERE table_schema='v3' AND table_name='conditions'
...
\echo '--- 許諾料の扱い（A-048。列 1 と CHECK 1 で 2 であること） ---'
SELECT (SELECT count(*) FROM information_schema.columns
         WHERE table_schema='v3' AND table_name='conditions' AND column_name='license_fee_basis')
     + (SELECT count(*) FROM pg_constraint
         WHERE conrelid='v3.conditions'::regclass AND conname='conditions_license_fee_basis_chk') AS 列とCHECK;

\echo '--- 担当者の英語表記（A-049。列 2 であること） ---'
...
\echo '--- 依頼の受付箱（A-052。メールの列まであれば 4 であること） ---'
SELECT count(*) AS 列 FROM information_schema.columns
 WHERE table_schema='v3' AND table_name='intake_requests'
   AND column_name IN ('counterparty_id', 'email_thread_id', 'email_message_id', 'source_payload');

\echo '--- 稟議・関連当事者・資料アップロード（A-053〜055。表 7 と列 5 で 12 であること） ---'

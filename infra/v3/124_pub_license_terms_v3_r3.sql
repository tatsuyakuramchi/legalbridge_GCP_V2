-- =====================================================================
-- 124_pub_license_terms_v3_r3.sql（ops sql / Cloud SQL Studio 用）
--
--   出版等利用許諾条件書 V3 ひな形（pub_license_terms_v3）の改訂 r3。
--   ページの割り付けを直す（r2 の本文・項目はそのまま）。
--
--   本番の ARC-PUBT-2026-1001 は A4 横 4 ページになり、2 ページ目は一覧の
--   注記 1 行だけ、3〜4 ページ目は 2 段組が途中で割れて左段が空だった。
--   原因は 3 つ：
--     ・一覧の下の注記が表からはぐれて次ページに落ちる → 注記を表の上に出す
--     ・条文の前に強制改ページ（page-break-before: always）→ やめる。一覧の
--       続きから流す
--     ・条文の 2 段組が flex（左右に分けた箱）→ ページをまたぐと片方だけ
--       次ページに行く。CSS の段組（column-count: 2）にして、上から順に
--       左段→右段→次ページ と流す。表と条は段の途中で割らない
--
--   123 と同じ作り。目印 r3 の版が既に current なら何もしない。古い版は残る。
--
--   実行: Cloud SQL Studio にそのまま貼る／ローカルは
--         docker compose run --rm ops sql /v3/124_pub_license_terms_v3_r3.sql
--   戻すとき（版id は【3】の一覧に出る）:
...
<style>
  ...
  /* 条文は CSS の段組。左段→右段→次ページ の順に流れ、ページをまたいでも
     片方の段だけが飛ぶことがない。条は段の途中で割れてよい（一覧の直後の
     余白から流し始めるため）が、表の行と、見出しとその直後は割らない。 */
  .two { column-count: 2; column-gap: 8mm; column-fill: auto; margin-top: 2mm; }
  ...
  .sign { display: flex; gap: 10mm; margin-top: 4mm; column-span: all; page-break-inside: avoid; break-inside: avoid; }
...
<p class="lead">一覧の1行が作品1点。紙・電子の料率と独占区分は、それぞれの媒体について甲乙間で合意した条件を示す。「—」はその媒体の許諾が無いことを示す。</p>
<table class="titles">
  <thead>
    <tr>
      <th class="n">No.</th>
...
{{!-- 署名欄は段組の中で両段にまたがる（column-span: all）。段組の外に置くと、
     段組がページの下まで伸びて署名欄だけ次ページに落ちる。またがる要素の
     前で段は揃うので、条文の下に余白があればそこに収まる。 --}}
{{#if showSignature}}
<div class="sign">
...
<div class="footer">{{docNo}}　出版等利用許諾条件書</div>
         ...
         '[]'::jsonb,
         'r3：ページ割りを直す（注記を表の上へ・強制改ページをやめる・2段組を CSS の段組に）',
         'infra/v3/124'
    FROM t
   ...
   WHERE NOT EXISTS (
           SELECT 1 FROM v3.document_template_versions v
            WHERE v.id = t.current_version_id
              AND position('<!-- pub_license_terms_v3 r3 -->' in v.html_source) > 0)

-- =====================================================================
-- 131_pub_license_terms_derivative_scope.sql（ops sql / Cloud SQL Studio 用）
--
--   「翻訳の扱い＝二次的著作物」のときの書き方を直す（A-036）。
--
--   130 までは、二次的著作物を選んでも第２条 許諾内容に「翻訳版」の行があり、
--   「甲は乙に対し…翻訳物を出版・配信することを許諾する」と書いていた。
--   これでは本条件書で翻訳まで許諾したように読める。翻訳は二次的著作物だと
--   言う取引先は、まさにそこを本条件書の外に置きたい。
--
--   二次的著作物を選んだときは、翻訳を第２条の「二次利用」に寄せる。
--     ・第２条の「翻訳版」「翻訳物の著作権」の行は出さない
--     ・「二次利用」＝ 翻訳（二次的著作物の作成）及び翻訳物の出版・配信、
--       映像化・商品化その他の二次利用は、いずれも本条件書の対象外とし、
--       甲乙の別途合意による。翻訳権その他 27 条・28 条の権利は甲に留保され、
--       本条件書による許諾には含まれない。料率は一覧の行を「目安」とする
--     ・第３条の「終了後の翻訳物」、第４条・第５条の翻訳の行も出さない
--       （許諾していないものの対価と支払を書くと、また許諾に読める）
--     ・一覧の「翻訳版」の行は料率だけ出す。別途合意は条文で常に要るので、
--       行に「別途合意 要／不要」は出さない
--     ・第７条は「別途合意により翻訳物を出版するときは、原著作物の題号及び
--       原著作者名を表示する」に直す
--   既定の「再許諾」を選んだときの本文は 130 と同じで、何も変わらない。
--
--   2 本のひな形の両方を改訂する。
--     pub_license_terms_v3        … 一覧形式（作品が数点）      r8 → r9
--     pub_license_terms_v3_annex  … 別紙形式（作品が数十点）    r4 → r5
--
--   先に 130 を流しておくこと（この SQL は 130 の本文を土台にしている）。
--
--   何度流しても同じ結果（目印の版が current なら何もしない）。実行:
--     Cloud SQL Studio にそのまま貼る／ローカルは
--     docker compose run --rm ops sql /v3/131_pub_license_terms_derivative_scope.sql
--
--   戻すとき（版id は【3】の一覧に出る）:
...
<p class="lead">一覧の1行が作品1点（備考がある作品は同じNo.の下に続ける）。紙・電子の料率と独占区分は、それぞれの媒体について甲乙間で合意した条件を示す。「—」はその媒体の許諾が無いことを示す。{{#if hasTranslationConditions}}{{#if translationDerivative}}翻訳の条件を定めた作品は、同じNo.の下の「{{translationLabel}}」の行に、翻訳について別途合意するときの料率の目安を示す（第２条）。{{else}}翻訳版の再許諾がある作品は、同じNo.の下の「{{translationLabel}}」の行に、紙・電子の料率と別途合意の要否を示す（第２条・第４条）。{{/if}}{{/if}}</p>
<table class="titles">
  <thead>
    <tr>
      <th class="n">No.</th>
  ...
  <tbody>
    ...
    <tr>
      <td class="c"></td>
      <td class="note" colspan="5">{{#if hasTranslation}}<span class="lbl">{{translationLabel}}</span>{{translation}}{{#unless translationDerivative}}（別途合意 {{translationConsent}}）{{/unless}}{{/if}}{{#if note}}<span class="lbl">備考</span>{{note}}{{/if}}</td>
    </tr>
    {{/if}}
  </tbody>
  {{/each}}
...
<div class="footer">{{docNo}}　出版等利用許諾条件書</div>
         ...
         '[]'::jsonb,
         'r9：二次的著作物のときは翻訳を許諾内容から外し、二次利用（別途合意）に寄せる',
         'infra/v3/131'
    FROM t
   WHERE NOT EXISTS (
           SELECT 1 FROM v3.document_template_versions v
            WHERE v.id = t.current_version_id
              AND position('<!-- pub_license_terms_v3 r9 -->' in v.html_source) > 0)
...
SELECT p.template_key AS キー, p.new_version::text AS 新しい版id, p.version_no::text AS 版番号
  FROM pointed p
UNION ALL
SELECT '—', '0 件', '同じ本文（r9）が既に使われています。何もしていません'
...
made AS (
  ...
  SELECT t.id,
         (SELECT COALESCE(max(x.version_no), 0) + 1
            FROM v3.document_template_versions x WHERE x.template_id = t.id),
         $html$<!-- pub_license_terms_v3_annex r5 -->
<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="utf-8">
...
<div class="annex">
<h2>別紙1　対象著作物一覧</h2>
<p class="cap">{{docNo}}　全 {{titleCount}} 点。1行が作品1点（備考がある作品は同じNo.の下に続ける）。紙・電子の料率と独占区分は、それぞれの媒体について甲乙間で合意した条件を示す。「—」はその媒体の許諾が無いことを示す。{{#if hasTranslationConditions}}{{#if translationDerivative}}翻訳の条件を定めた作品は、同じNo.の下の「{{translationLabel}}」の行に、翻訳について別途合意するときの料率の目安を示す（第２条）。{{else}}翻訳版の再許諾がある作品は、同じNo.の下の「{{translationLabel}}」の行に、紙・電子の料率と別途合意の要否を示す（第２条・第４条）。{{/if}}{{/if}}</p>
<table class="titles">
  <thead>
    <tr>
      <th class="n">No.</th>
...
<div class="footer">{{docNo}}　出版等利用許諾条件書</div>
         ...
         '[]'::jsonb,
         'r5：二次的著作物のときは翻訳を許諾内容から外し、二次利用（別途合意）に寄せる',
         'infra/v3/131'
    FROM t
   WHERE NOT EXISTS (
           SELECT 1 FROM v3.document_template_versions v
            WHERE v.id = t.current_version_id
              AND position('<!-- pub_license_terms_v3_annex r5 -->' in v.html_source) > 0)
  RETURNING id, template_id, version_no
),
pointed AS (
  UPDATE v3.document_templates d
...
SELECT p.template_key AS キー, p.new_version::text AS 新しい版id, p.version_no::text AS 版番号
  FROM pointed p
UNION ALL
SELECT '—', '0 件', '同じ本文（r5）が既に使われています。何もしていません'
 WHERE NOT EXISTS (SELECT 1 FROM pointed);

-- ---------------------------------------------------------------------
-- 【3】確認。2 本とも「翻訳は二次利用へ」「許諾の行は無い」が true になっていること。
-- ---------------------------------------------------------------------
SELECT t.template_key AS キー, t.label AS 名前, v.id AS 版id, v.version_no AS 版番号,
       ...
       (position('本条件書の対象外とし、甲乙の別途合意による' in v.html_source) > 0) AS 翻訳は二次利用へ,
       (position('<tr><th>翻訳物の著作権</th>' in v.html_source) = 0)               AS 許諾の行は無い

  ...
  IF to_regclass('public.ringi_documents') IS NOT NULL THEN
    EXECUTE $q$
      ...
      SELECT nr.id, 'document', nd.id, COALESCE(rd.linked_at, now()), 'migration:050'
        ...
        JOIN v3.ringi nr ON nr.legacy_id::text = rd.ringi_id::text
        JOIN v3.documents nd ON nd.legacy_id::text = rd.document_id::text
  ...
  IF to_regclass('public.ringi_works') IS NOT NULL THEN
    EXECUTE $q$
      ...
      SELECT nr.id, 'work', w.id, 'migration:050'
        ...
        JOIN v3.ringi nr ON nr.legacy_id::text = rw.ringi_id::text
        JOIN v3.works w ON w.legacy_table = 'works' AND w.legacy_id::text = rw.work_id::text
      ON CONFLICT DO NOTHING
    $q$;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    EXECUTE $q$
      ...
      SELECT nr.id, 'agreement', a.id, 'migration:050'
        ...
        JOIN v3.ringi nr ON nr.legacy_id::text = c.ringi_id::text
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    EXECUTE $q$
      ...
      SELECT nr.id, 'condition', nc.id, 'migration:050'
        ...
        JOIN v3.ringi nr ON nr.legacy_id::text = cl.ringi_id::text
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    EXECUTE $q$
      UPDATE v3.parties p
       ...
       WHERE p.legacy_id::text = v.id::text AND v.rpt_entity IS TRUE AND NOT p.rpt_entity
    $q$;
    GET DIAGNOSTICS n = ROW_COUNT;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    EXECUTE $q$
      UPDATE v3.parties p
        ...
        FROM public.vendors v
       WHERE p.legacy_id::text = v.id::text AND v.related_party IS TRUE AND NOT p.related_party
    $q$;
  END IF;
  ...
  EXECUTE $q$
    ...
    SELECT o.officer_key, o.name,
           -- V1 の staff_id は文字列（職員コードか職員の番号）。どちらでも当てる。
           (SELECT s.id FROM v3.staff s
             WHERE NULLIF(btrim(o.staff_id::text), '') IS NOT NULL
               AND (s.staff_code = btrim(o.staff_id::text) OR s.legacy_id::text = btrim(o.staff_id::text))
             ORDER BY (s.staff_code = btrim(o.staff_id::text)) DESC LIMIT 1),
           o.voided_at, CASE WHEN o.id::text ~ '^[0-9]{1,9}$' THEN o.id::text::int END,
  ...
  IF to_regclass('public.officer_roles') IS NOT NULL THEN
    EXECUTE $q$
      ...
      SELECT no.id, p.id, r.title, COALESCE(r.is_director, true)
        ...
        JOIN v3.officers no ON no.legacy_id::text = r.officer_id::text
        JOIN v3.parties p ON p.legacy_id::text = r.entity_id::text
  ...
  IF to_regclass('public.vendor_shareholdings') IS NOT NULL THEN
    EXECUTE $q$
      ...
      SELECT p.id,
        ...
        JOIN v3.parties p ON p.legacy_id::text = sh.entity_id::text
        LEFT JOIN v3.parties hp ON hp.legacy_id::text = sh.holder_entity_id::text
        LEFT JOIN v3.officers ho ON ho.legacy_id::text = sh.holder_officer_id::text
  ...
  IF to_regclass('public.ringi_related_party') IS NOT NULL THEN
    EXECUTE $q$
      ...
      SELECT nr.id, p.id, x.meeting_date, COALESCE(NULLIF(x.txn_type, ''), 'その他'),
        ...
        JOIN v3.ringi nr ON nr.legacy_id::text = x.ringi_id::text
        LEFT JOIN v3.parties p ON p.legacy_id::text = x.entity_id::text

-- =====================================================================
-- 125_pub_license_terms_v3_r4.sql（ops sql / Cloud SQL Studio 用）
--
--   出版等利用許諾条件書 V3 ひな形（pub_license_terms_v3）の改訂 r4。
--   A4 縦・1 段組にする（r3 までは A4 横・2 段組）。
--
--   ・条文は普通の契約書と同じ 1 段（1 行 45 字前後）で上から流す
--   ・第１条の一覧は「備考」の列をやめ、備考がある作品だけ同じ No. の下に
--     2 行目として出す（縦幅に 7 列を詰めると作品名が 3〜4 行に折れるため）。
--     作品の 1 行目と備考の行はページをまたがない
--   ・注記は表の上、署名欄は末尾（r3 と同じ）。本文の項目と値の出どころは同じ
--
--   124 と同じ作り。目印 r4 の版が既に current なら何もしない。古い版は残る。
--
--   実行: Cloud SQL Studio にそのまま貼る／ローカルは
--         docker compose run --rm ops sql /v3/125_pub_license_terms_v3_r4.sql
--   戻すとき（版id は【3】の一覧に出る）:
...
<p class="lead">一覧の1行が作品1点（備考がある作品は同じNo.の下に続ける）。紙・電子の料率と独占区分は、それぞれの媒体について甲乙間で合意した条件を示す。「—」はその媒体の許諾が無いことを示す。</p>
<table class="titles">
  <thead>
    <tr>
      <th class="n">No.</th>
...
<div class="footer">{{docNo}}　出版等利用許諾条件書</div>
         ...
         '[]'::jsonb,
         'r4：A4 縦・1 段組。一覧の備考は No. の下の 2 行目に',
         'infra/v3/125'
    FROM t

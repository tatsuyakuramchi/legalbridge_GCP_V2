-- =====================================================================
-- 126_pub_license_terms_v3_r5.sql（ops sql / Cloud SQL Studio 用）
--
--   出版等利用許諾条件書 V3 ひな形（pub_license_terms_v3）の改訂 r5。
--   対象著作物の一覧を本文から出して「別紙1」にする。
--
--   r4 までは第１条がそのまま一覧表だったので、作品 80 点の条件書では
--   条文の前に表が 4 ページ半続き、第２条が 5 ページ目、署名欄が 6 ページ目に
--   来ていた。相手方は条文から読むので、読み順が逆だった。
--
--   ・第１条は「別紙1のとおり（全 N 点）」の要約にする（N は titleCount）
--   ・本文（頭書き〜第１０条・署名欄）は 2 ページで完結する
--   ・一覧は改ページして「別紙1 対象著作物一覧」として続ける。列は r4 と同じ
--   ・条文の参照先を「第１条一覧」から「別紙1」に直す（一覧が本文から出るため）
--   ・文書番号を全ページの右下に出す（別紙だけ抜き取られても出どころが分かる）
--
--   項目と値の出どころは r4 と同じ。作品数（titleCount）だけコードが足す
--   （src/server/documents/pub-terms.ts）。
--
--   125 と同じ作り。目印 r5 の版が既に current なら何もしない。古い版は残る
--   （決定済みの条件書は版が凍っているので変わらない）。
--
--   実行: Cloud SQL Studio にそのまま貼る／ローカルは
--         docker compose run --rm ops sql /v3/126_pub_license_terms_v3_r5.sql
--   戻すとき（版id は【3】の一覧に出る）:
...
<div class="footer">{{docNo}}　出版等利用許諾条件書</div>
         ...
         '[]'::jsonb,
         'r5：対象著作物の一覧を別紙1に出す（本文は条文だけで 2 ページ）',
         'infra/v3/126'
    FROM t
...
SELECT t.template_key AS キー, t.label AS 名前, t.number_prefix AS 採番,
       ...
       (position('size: A4 portrait' in v.html_source) > 0) AS 縦向き,
       (position('{{#each titles}}' in v.html_source) > 0)   AS 一覧あり,
       (position('別紙1　対象著作物一覧' in v.html_source) > 0) AS 別紙あり

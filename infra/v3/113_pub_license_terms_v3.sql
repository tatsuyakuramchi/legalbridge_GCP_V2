-- =====================================================================
-- 113_pub_license_terms_v3.sql（ops sql / Cloud SQL Studio 用）
--
--   出版等利用許諾条件書の V3 ひな形（pub_license_terms_v3）を登録する。
--
--   V2 の pub_license_terms は残す（is_active のまま。過去の文書がその版を
--   指している）。V3 のひな形は別のキーで足し、採番は同じ PUBT を使う
--   （ARC-PUBT-2026-0041 の並びが続く）。
--
--   ひな形の項目（variables）はコードが持つ（src/server/documents/pub-terms.ts）。
--   ここは本文だけを運ぶ。本文は A4 横で、1ページ目に対象著作物の一覧、
--   その後ろに条文が2段組で続く。
--
--   本文の値の出どころ：
--     ・対象著作物の一覧 … 条件明細（作品1点＝紙・電子の2本）から。料率と
--       独占区分は台帳の写しで、紙の上では直せない
--     ・許諾者（甲）  … 取引先（著作権者）。振込先も取引先の口座
--     ・被許諾者（乙）… 当社
--     ・対価の条     … 固定文言。一覧の「紙」「電子」欄を参照する
--
--   何度流しても同じ結果になる。同じ目印（r1）の版が既に current なら
--   何もしない。本文を改訂するときは目印を r2 にして流すと新しい版が
--   できて current がそちらを向く。古い版は残る（発行済みの文書は凍って
--   いるので変わらない）。
--
--   戻すとき（版id は【3】の一覧に出る）:
...
{{#if showSignature}}
<div class="sign">
  <div><div class="who">甲</div>{{licensorName}}{{#if licensorRep}}　{{licensorRep}}{{/if}}</div>
  <div><div class="who">乙</div>{{licenseeName}}{{#if licenseeRep}}　{{licenseeRep}}{{/if}}</div>
</div>
{{/if}}
...
<div class="footer">{{docNo}}　出版等利用許諾条件書</div>
         ...
         '[]'::jsonb,
         '初版：一覧形式（紙・電子の料率を作品ごとに）、A4横、対価は固定文言',
         'infra/v3/113'
    FROM t
   ...
   WHERE NOT EXISTS (
           SELECT 1 FROM v3.document_template_versions v
            WHERE v.id = t.current_version_id
              AND position('<!-- pub_license_terms_v3 r1 -->' in v.html_source) > 0)
...
SELECT p.template_key AS キー, p.new_version::text AS 新しい版id, p.version_no::text AS 版番号
  FROM pointed p
UNION ALL
SELECT '—', '0 件', '同じ本文（r1）が既に使われています。何もしていません'

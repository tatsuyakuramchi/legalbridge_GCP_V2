-- =====================================================================
-- 130_pub_license_terms_note_row.sql（ops sql / Cloud SQL Studio 用）
--
--   翻訳版の再許諾を、一覧の「列」ではなく「備考と同じ全幅の行」に出す（A-035）。
--
--   129 までは対象著作物の一覧に「翻訳版再許諾　料率／別途合意」の列を足して
--   いた。幅が 22mm しか取れないので「紙 50%」「電子 40%」「別途合意 要」を
--   3 行に折って詰め、そのぶん著作権表示（46mm→38mm）と共同著作・第三者権利
--   （28mm→24mm）を削っていた。著作権表示が「© 2026 著／者名」のように折れる。
--
--   翻訳版がある作品は、同じ No. の下に 1 行足して、そこに 1 文で書く。
--     翻訳版再許諾  紙 50%／電子 40%（別途合意 要）  備考  初版3000部…
--   備考がある作品は同じ行にまとめるので、備考つきの作品では行は増えない。
--   表の列は 6 列（翻訳版が無いときと同じ幅）に戻る。
--
--   条文の指し先も「欄」から「行」に直す（第２条・第４条）。翻訳の立て付けの
--   切替（再許諾／二次的著作物。A-034）はそのまま効く。
--
--   2 本のひな形の両方を改訂する。
--     pub_license_terms_v3        … 一覧形式（作品が数点）      r7 → r8
--     pub_license_terms_v3_annex  … 別紙形式（作品が数十点）    r3 → r4
--
--   先に 129 を流しておくこと（この SQL は 129 の本文を土台にしている）。
--
--   何度流しても同じ結果（目印の版が current なら何もしない）。実行:
--     Cloud SQL Studio にそのまま貼る／ローカルは
--     docker compose run --rm ops sql /v3/130_pub_license_terms_note_row.sql
--
--   戻すとき（版id は【3】の一覧に出る）:
...
<style>
  ...
  .titles td.note .lbl + .lbl { margin-left: .6em; }
...
<p class="lead">一覧の1行が作品1点（備考がある作品は同じNo.の下に続ける）。紙・電子の料率と独占区分は、それぞれの媒体について甲乙間で合意した条件を示す。「—」はその媒体の許諾が無いことを示す。{{#if hasTranslationConditions}}{{#if translationDerivative}}翻訳物（二次的著作物）の出版・配信{{else}}翻訳版の再許諾{{/if}}がある作品は、同じNo.の下の「{{translationLabel}}」の行に、紙・電子の料率と別途合意の要否を示す（第２条・第４条）。{{/if}}</p>
<table class="titles">
  <thead>
    <tr>
      <th class="n">No.</th>
  ...
  <tbody>
    ...
    <tr>
      <td class="c"></td>
      <td class="note" colspan="5">{{#if hasTranslation}}<span class="lbl">{{translationLabel}}</span>{{translation}}（別途合意 {{translationConsent}}）{{/if}}{{#if note}}<span class="lbl">備考</span>{{note}}{{/if}}</td>
    </tr>
    {{/if}}
  </tbody>
  {{/each}}
...
<table class="kv">
  ...
  <tr><th>{{translationLabel}}</th><td>乙が{{#if translationDerivative}}翻訳物の出版・配信につき第三者から{{else}}再許諾先から{{/if}}受領する対価（税抜）× 料率（第１条一覧の「{{translationLabel}}」の行）。紙と電子で料率が異なる場合は、同行の媒体ごとの率による。当該の行が無い作品には翻訳版の許諾料は発生しない。算定の細目に別段の定めをするときは、一覧の備考欄又は特記事項による。</td></tr>
...
<div class="footer">{{docNo}}　出版等利用許諾条件書</div>
         ...
         '[]'::jsonb,
         'r8：翻訳版の再許諾を一覧の列ではなく、備考と同じ全幅の行に出す',
         'infra/v3/130'
    FROM t
   WHERE NOT EXISTS (
           SELECT 1 FROM v3.document_template_versions v
            WHERE v.id = t.current_version_id
              AND position('<!-- pub_license_terms_v3 r8 -->' in v.html_source) > 0)
...
SELECT p.template_key AS キー, p.new_version::text AS 新しい版id, p.version_no::text AS 版番号
  FROM pointed p
UNION ALL
SELECT '—', '0 件', '同じ本文（r8）が既に使われています。何もしていません'
...
made AS (
  ...
  SELECT t.id,
         (SELECT COALESCE(max(x.version_no), 0) + 1
            FROM v3.document_template_versions x WHERE x.template_id = t.id),
         $html$<!-- pub_license_terms_v3_annex r4 -->
<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="utf-8">
...
<table class="kv">
  ...
  {{#if hasTranslation}}
  <tr><th>翻訳版</th><td>{{#if translationDerivative}}甲は乙に対し、対象著作物を翻訳し、又は第三者をして翻訳させ、その翻訳物（二次的著作物）を出版・配信することを許諾する（著作権法第27条・第28条）。翻訳に伴う改変は翻訳に通常必要な範囲に限り、表題の変更・抄訳その他これを超える改変は甲の事前の書面による承諾を要する。{{#if hasTranslationConditions}}対象作品と許諾料の料率は別紙1の各作品の「{{translationLabel}}」の行による。{{/if}}{{#if translationConsentMixed}}同行に「別途合意 要」とある作品は、翻訳ごとに翻訳者・言語・翻訳物を出版する第三者及び条件について甲乙が別途書面で合意する。同行に「別途合意 不要」とある作品は、本条件書に定める条件の範囲内であれば甲の個別の事前承諾を要しない。{{else}}{{#if translationConsentRequired}}翻訳ごとに、翻訳者・言語・翻訳物を出版する第三者及び条件について甲乙が別途書面で合意する。{{else}}本条件書に定める条件の範囲内で行う翻訳については、甲の個別の事前承諾を要しない。{{/if}}{{/if}}いずれの場合も、乙は翻訳ごとに翻訳者・言語・翻訳物を出版する第三者及び受領する対価を甲へ通知し、甲の求めに応じて翻訳原稿を提示する。{{else}}乙が第三者に再許諾して行わせる翻訳版の出版。{{#if hasTranslationConditions}}対象作品と料率は別紙1の各作品の「{{translationLabel}}」の行による。{{/if}}{{#if translationConsentMixed}}同行に「別途合意 要」とある作品は、再許諾ごとに再許諾先・言語・条件について甲乙が別途書面で合意する。同行に「別途合意 不要」とある作品は、本条件書に定める条件の範囲内であれば甲の個別の事前承諾を要しない。{{else}}{{#if translationConsentRequired}}再許諾ごとに、再許諾先・言語・条件について甲乙が別途書面で合意する。{{else}}本条件書に定める条件の範囲内で行う再許諾については、甲の個別の事前承諾を要しない。{{/if}}{{/if}}いずれの場合も、乙は再許諾ごとに再許諾先・言語・受領する対価を甲へ通知する。{{/if}}</td></tr>
  {{#if translationDerivative}}
  <tr><th>翻訳物の著作権</th><td>翻訳物のうち翻訳に係る部分の著作権は翻訳者に帰属する。乙は、翻訳者又は翻訳物を出版する第三者から、本条件書に基づく利用に必要な権利の譲渡又は利用許諾を受けるものとする。甲は、翻訳物の利用に関し著作権法第28条に基づく権利を留保する。</td></tr>
  {{/if}}
  {{/if}}
...
<table class="kv">
  ...
  {{#if hasTranslationConditions}}
  <tr><th>{{translationLabel}}</th><td>乙が{{#if translationDerivative}}翻訳物の出版・配信につき第三者から{{else}}再許諾先から{{/if}}受領する対価（税抜）× 料率（別紙1の「{{translationLabel}}」の行）。紙と電子で料率が異なる場合は、同行の媒体ごとの率による。当該の行が無い作品には翻訳版の許諾料は発生しない。算定の細目に別段の定めをするときは、一覧の備考欄又は特記事項による。</td></tr>
  {{else}}{{#if hasTranslation}}
  <tr><th>翻訳版</th><td>乙が再許諾先から受領する対価（税抜）の {{translationShare}}。</td></tr>
  {{/if}}{{/if}}
</table>
...
<div class="annex">
<h2>別紙1　対象著作物一覧</h2>
<p class="cap">{{docNo}}　全 {{titleCount}} 点。1行が作品1点（備考がある作品は同じNo.の下に続ける）。紙・電子の料率と独占区分は、それぞれの媒体について甲乙間で合意した条件を示す。「—」はその媒体の許諾が無いことを示す。{{#if hasTranslationConditions}}{{#if translationDerivative}}翻訳物（二次的著作物）の出版・配信{{else}}翻訳版の再許諾{{/if}}がある作品は、同じNo.の下の「{{translationLabel}}」の行に、紙・電子の料率と別途合意の要否を示す（第２条・第４条）。{{/if}}</p>
<table class="titles">
  <thead>
    <tr>
      <th class="n">No.</th>
...
<div class="footer">{{docNo}}　出版等利用許諾条件書</div>
         ...
         '[]'::jsonb,
         'r4：翻訳版の再許諾を別紙1の列ではなく、備考と同じ全幅の行に出す',
         'infra/v3/130'
    FROM t
   WHERE NOT EXISTS (
           SELECT 1 FROM v3.document_template_versions v
            WHERE v.id = t.current_version_id
              AND position('<!-- pub_license_terms_v3_annex r4 -->' in v.html_source) > 0)
  RETURNING id, template_id, version_no
),
pointed AS (
  UPDATE v3.document_templates d
...
SELECT p.template_key AS キー, p.new_version::text AS 新しい版id, p.version_no::text AS 版番号
  FROM pointed p
UNION ALL
SELECT '—', '0 件', '同じ本文（r4）が既に使われています。何もしていません'
 WHERE NOT EXISTS (SELECT 1 FROM pointed);

-- ---------------------------------------------------------------------
-- 【3】確認。2 本とも「翻訳版は行で出る」「列は無い」が true になっていること。
-- ---------------------------------------------------------------------
SELECT t.template_key AS キー, t.label AS 名前, v.id AS 版id, v.version_no AS 版番号,
       ...
       (position('hasNoteRow' in v.html_source) > 0)             AS 翻訳版は行で出る,
       (position('class="trans"' in v.html_source) = 0)          AS 列は無い

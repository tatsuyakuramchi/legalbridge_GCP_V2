...
--
--   消すのは3か所。どれか1つでも残ると紙に出るので、まとめて消す。
--     1. 受領情報の {{#if desiredDeadline}} …… {{/if}}（2か所ある。
--        receiptRows がある枝の div と、無い枝の表の行）
--     2. 表の枡で組まれているもの（td / th）
--     3. 下の合計の枠（result-box）。ここは div で組まれていて、
--        「仕切り線 + 見出し + 値」の3枚で1列になっている。仕切り線も
--        一緒に消さないと、枠の右端に線だけが残る。
--
  ...
  SELECT s.*,
         regexp_replace(
           regexp_replace(
             ...
             '\s*<t[dh][^>]*>[^<]*希望納期[^<]*</t[dh]>', '', 'g'),
             -- 2b. 値の枡。
             '\s*<t[dh][^>]*>\s*\{\{desiredDeadline\}\}\s*</t[dh]>', '', 'g'),
           -- 3. 合計の枠の1列（div 組み）。直前の仕切り線ごと。
           --    量指定子は \s* と [^>]* だけ。どちらも < や > を跨げないので、
           --    式全体の貪欲さに左右されない（.*? の失敗を繰り返さない）。
           '(?:\s*<div[^>]*class="result-divider"[^>]*>\s*</div>)?'
           || '\s*<div[^>]*class="result-item"[^>]*>'
           || '\s*<div[^>]*>希望納期</div>'
           || '\s*<div[^>]*>\s*\{\{desiredDeadline\}\}\s*</div>'
           || '\s*</div>', '', 'g') AS html
    FROM src s
),
made AS (

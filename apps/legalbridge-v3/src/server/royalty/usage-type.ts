...
/**
 * 受領額を割り戻すときの税率。国内の消費税に合わせて 10% 固定。
 *
 * **これは決めごとで、直し忘れではない（2026-09 合意）。**
 *
 * 受領元の国によって税率は違う（台湾 5%・韓国 10%・EU は国ごとに 20% 前後）。
 * 台湾からの税込 105 を 1.1 で割ると 95 になり、本当の税別 100 より小さい
 * 基礎で計算することになる。作者への支払いが少なくなる向きの誤差で、紙にも
 * 「税込 ÷ 1.1」と印字されるので、相手が検算すると合わない。
 *
 * それを承知のうえで、当面は 10% 固定で運用する。国別に持たせるには
 * 取引先か条件に税率の欄が要り、既存のアウト条件すべてに値を入れて回る
 * ことになる。実際に困ってから入れる。
 *
 * 直すときは、ここを国別の表に替えるのではなく、税率の出どころ
 * （取引先か、アウト条件か）を先に決めること。
 */

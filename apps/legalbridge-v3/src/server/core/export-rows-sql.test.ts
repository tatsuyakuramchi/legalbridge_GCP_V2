...
test("取り込みは取り出した CSV 以外を名指しで断る", () => {
  // 取り込み先に関係ない CSV が混ざると、文字コードの違いで分かりにくく落ちる。
  // 見出しで選り分けて、混ざっているファイル名を出して止める。
  assert.match(ops(), /094_export_rows\.sql の結果ではない CSV が混ざっています/,
    "取り込み側に見出しの確認が無い");
});

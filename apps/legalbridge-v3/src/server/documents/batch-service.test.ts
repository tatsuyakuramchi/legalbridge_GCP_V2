...
test("雛形の見出しは列の定義から出す（BOM 付き）。例は作品違いの2行", () => {
  ...
  // 支払条件として読めず、支払期日が空のまま出ていた。
  assert.ok(csv.includes("契約形式,支払条件"));
});

test("旧い雛形の「契約種別・支払条件」は契約形式として読む", () => {
  // その欄に書かれていたのは「請負」、つまり契約形式のほう。
  const rows = readRows(`${HEAD}
VD-00317,合同会社アトリエ蒼,,,,,原稿制作,一式,1,135000,検収後,2026-11-30,2026-12-31,請負,発注者,,,固定額,`);
  assert.equal(rows[0].item.payment_terms, "請負", "紙に出る契約種別は契約形式");
  assert.equal(rows[0].paymentTerms, null, "支払条件の列は別。旧い雛形には無い");
});

test("新しい雛形は契約形式と支払条件を別々に読む", () => {
  const head = "取引先コード,取引先名,作品コード,作品名,契約番号,条件名,品目・業務名,仕様・成果物,数量,"
    + "単価（税抜）,起点,納期,支払日,契約形式,支払条件,成果物の帰属先,発注署名欄,承諾署名欄,支払方法,備考,"
    + "一括修正,修正理由";
  const rows = readRows(`${head}
VD-00317,合同会社アトリエ蒼,,,,,原稿制作,一式,1,135000,検収後,2026-11-30,2026-12-31,請負,月末締め翌月末払い,発注者,,,固定額,`);
  assert.equal(rows[0].item.payment_terms, "請負");
  assert.equal(rows[0].paymentTerms, "月末締め翌月末払い");
});

...
const responder = (options: Options = {}) => (text: string): Array<Record<string, unknown>> | undefined => {
  if (text.includes("FROM statements s")) {
    return [{ id: 800, condition_id: 5, currency: "JPY", net_amount: 612000, tax_amount: 61200,
...
test("計算書から支払を起こし、必ず条件と実績に割り当てる", async () => {
  ...
  const result = await service.createFromStatement(800, "kuramochi");
...
test("期日を61日以降にすると超過として記録に残る", async () => {
  ...
  const result = await service.createFromStatement(800, "kuramochi", { dueOn: "2026-08-27" });
...
test("同じ実績に二重の支払は作らない", async () => {
  ...
  await assert.rejects(() => service.createFromStatement(800, "x"),

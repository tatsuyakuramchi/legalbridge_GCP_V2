...
class DocumentRepo implements DocumentRegistryRepository {
  ...
  async findByNumber(documentNumber: string) { return documentNumber === document.documentNumber ? document : null; }
  async versionHistory() { return []; }
  async setDriveLink() {}
}

class BacklogFake implements BacklogDocumentClient {
...
function app(client: BacklogDocumentClient | null, live = false) {
  const app = express();

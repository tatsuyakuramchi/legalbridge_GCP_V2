...
export async function persistRoyaltyNormalization(
  ...
  // recordConditionEvent=false: 消化イベントは呼び出し側（finalization-routes の
  // royaltyEventInputsFromStatement）が記帳する＝ここでは condition_events に書かない。
  options: { recordConditionEvent?: boolean } = {}
) {
  ...
  const conditionEventId = options.recordConditionEvent !== false && conditionLineId && actualRoyalty !== null

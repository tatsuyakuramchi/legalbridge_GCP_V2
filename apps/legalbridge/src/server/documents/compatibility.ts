...
function analyzeExpressions(htmlSource:string){
 ...
 for(const expression of expressions){
  ...
  for(const token of values){if(BUILTIN_HELPERS.has(token)||REGISTERED_HELPERS.has(token)||["true","false","null","undefined","this","VAR","xxx"].includes(token))continue;addVariable(variables,token)}
 }

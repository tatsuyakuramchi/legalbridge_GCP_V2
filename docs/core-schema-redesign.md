...
作業はすべて案件から入る。案件の `kind`（取引モデル）が、必須項目・検査・使えるテンプレートを決める。

| 取引モデル | 対象 | 段階 |
|---|---|---|
| `work` ライセンス | 権利の取得と展開 | 上限確認 → 条件合意 → 契約書 → 実績 → 計算書と分配 |
| `outsourcing` 業務委託 | 制作の発注 | 基本契約 → 文書 → 納品 → 検収 → 支払 |
| `single` 文書作成 | NDA・通知書・法務相談 | 受付 → 文書の用意 → 締結 → 完了 |

`single` は**条件を持たない案件**である。金銭条件も権利の移動も伴わない案件が実運用の1〜2割を占めるため、
条件を必須にしない設計が要る。

取引モデルは「何を扱うか」しか決めない。実際に手を動かす内容は `document_style`（進め方）が決める。

| 進め方 | 前提 | 文書の段階でやること |
|---|---|---|
| `counterparty_review` 他社文書レビュー型 | 相手方から文書が届く | 受け取った文書を取り込んで確認する |
| `own_draft` 自社ドラフト型 | ひな形に無い条件 | 自社で書いた文書を登録して発行する |
| `own_template` 自社テンプレートドラフト型 | 登録済みのひな形がある | ひな形を選んで発行する（条件から自動で埋まる） |

進め方は3つの取引モデルすべてに付く。文書を作らない案件は無いためである。
未設定でもよく（相手方から文書が来て初めて他社レビューだと分かることが多い）、
その場合は文書の段階が「進め方が未設定」として次にやることに出る。

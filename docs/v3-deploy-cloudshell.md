...
## 手順8.65：経理提出用 Excel を確かめる

V1 が出していた経理提出用エクセル（支払内容×8・立替金・小計・消費税・
源泉税・税引後・差引振込額・インボイス登録）を V3 から出す。**列名も順番も
V1 から変えていない**（提出先の運用を変えさせないため）。
...
| | V1・V2 | V3 |
|---|---|---|
| 1行の単位 | 文書（検収書・計算書） | **支払（payments）** |
| 金額の出どころ | 文書の `form_data`（JSON を別名キーで探索） | 支払・割当・条件の列 |
| 作る場所 | ブラウザ | サーバ（画面を開かなくても出せる） |

行の単位を支払にしたのは、V1 に支払という実体が無く文書が引き金だったから。
V3 は `payments` が支払日も振込額も源泉も持っているので、そちらが正しい単位。

**★ 出す前に「要確認」を0にする**

要確認が残っていても Excel は出るが、Excel の「要確認」列に理由が入る。

| 印 | 意味 | 直し方 |
|---|---|---|
| 割当なし | 支払がどの条件に対するものか未登録。**支払内容の欄が空欄で出る** | お金 → 支払 → 割当を入れる |
| 割当が支払額と不一致 | 割当の合計が支払額と違う。税区分の内訳に使えない | 同上（不足分を足す） |
| 源泉が計算値と不一致 | 保存された源泉と、税込×10.21%（100万超は20.42%）が違う | 支払の源泉を直す |

> **移行直後は「割当なし」が大量に出る。** V1 に割当に相当する表が無く、
> 移行で作れなかったため（`PAYMENT_UNALLOCATED` として運用 → データ品質にも
> 出ている）。割当が入るまで支払内容の欄は埋まらない。金額の列（小計・
> 消費税・源泉・振込額）は支払そのものが持っているので出る。

「出力済みにする」を押すと次の集計から外れる（V1 の「保留」と同じ考え方）。
記録先は監査記録（`export.accounting`）で、専用の台帳表は作っていない。
取り消しても記録は消えない（`export.accounting.undo` を足す）。監査記録は
追記専用で、ランタイムロールに DELETE 権限が無い。

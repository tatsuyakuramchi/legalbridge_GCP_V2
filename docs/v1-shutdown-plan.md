...
決定（2026-09-26）: `/法務検索` を V3 に取り込み、その後 V1 を完全に止めて V3 だけにする。
...
| 納期変更の依頼（`/api/intake/deadline-change-request`）→ 承認で反映 | 無し | ❓ 使っているなら 🔧 |
...
| V1 | V3 | 扱い |
|---|---|---|
| 発注書ごとの納期 7・3・1 日前と超過のアラート（依頼者 DM・部署チャンネル） | `jobs/daily.ts` は期限切れ・期限間近の一覧を1通にまとめて1か所へ | 🔧 納期のアラートを足す |
...
| 法務ガイド（`/portal`・`/guide`・`/g/:key`、版と閲覧制限つき） | 無し | ❓ |
| 関連当事者（RPT：`gas/RPT.gs`、取締役会の議題） | 無し | ❓ |
| 稟議（`/search/ringi`、稟議と文書の紐付け） | 無し | ❓ |
| 依頼者の資料アップロード（署名付きリンク、課題キーで紐付け） | 案件の Drive（社内の人だけ） | ❓ |
...
9. **V1 を止める**：Cloud Run の 3 サービスを削除（か最小インスタンス 0・呼び出し権限の剥奪）、
   GAS のデプロイを無効化、`release/api`・`release/worker` の Cloud Build トリガーを無効化。

...
| 2026-08-10 | 1-6 | 帳票への会社プロファイル差込（app_settings 参照・ハードコード解消・grant/env 追加なし） | LegalBridge_AI_GCP | ✅ |

**1-6 会社プロファイル差込**：runbook 残実装の最終項。`settings/company-profile.ts` 新設＝CompanyProfile 型＋DEFAULT（従来ハードコード値を既定に据える）＋loadCompanyProfile（app_settings の COMPANY_* 10 キーを読み、空値・表未整備 42P01・権限なし 42501 は既定へ縮退＝帳票差込を止めない）。配線＝①MasterDataPicker の company タイプ：PgMasterDataRepository に settings 注入・values を全 10 キー（name/name_kana/postal_code/address/tel/fax/rep/invoice_no/bank_info/seal_note）へ拡張＋クライアント company エイリアスに COMPANY_TEL/COMPANY_INVOICE_NO（適格請求書発行事業者番号）/COMPANY_BANK_INFO 等を追加＝文書フォームの自社差込が設定値で埋まる。②contract-intake ブリッジ：Licensor_氏名会社名／PARTY_A_NAME／payerCompany のハードコードを companyName 引数化し、routes が loadCompanyProfile で解決して注入。**追加 grant/env なし**（app_settings SELECT は grant 036 で付与済み・値の編集は 11-1 設定画面＝settings scope 点火で解禁）。tests 6件＝742緑・typecheck・build 緑。**これで runbook §1 残実装（1-1〜1-6）はすべて完了＝残るは点火メニューと業務判断のみ。**

| 2026-08-10 | Phase 9 | 9-7 完成形：Backlog Webhook 自動起票（legal_requests 作成＋状態同期・grant 046・BACKLOG_INTAKE_ENABLED） | LegalBridge_AI_GCP | ✅ |

**9-7 Backlog Webhook 自動起票（完成形）**：受信記録＋通知どまりだった 9-7 を V1 `/api/webhooks/backlog` 相当へ拡張。parsers＝parseBacklogIssueCreated に description/issueType.name を追加＋parseBacklogStatusChanged（type=2）＋BACKLOG_ISSUE_TYPE_TO_REQUEST_TYPE（V1 同一 18 種・未知→legal_consult）＋extractSlackMention。intake-repository に requestExists／setWorkflowStatus（UPDATE）。handler＝BACKLOG_INTAKE_ENABLED×readwrite で PgSlackIntakeRepository を注入し、課題追加（type=1）は legal_requests 既存（Slack 経由起票）なら「受付済み」遷移・無ければ自動取込（recordRequest→0103 トリガで matters 自動生成＝V2 案件モデル接続、notes に details/issueTypeName/source='backlog-webhook'）、ステータス変更（type=2）は issue_workflows.current_status_name 同期（UPDATE 自体べき等＝受信台帳を通さない）。42501 は forbidden 計上で受信は成功（再送誘発しない）。**grant 046**＝issue_workflows の列レベル UPDATE（current_status_name/updated_at）のみ・guard で 044 適用済み検査・token GRANT_PRODUCTION_BACKLOG_INTAKE。verify ゲート（write-test 限定＋BACKLOG_WEBHOOK_TOKEN_SECRET 必須＋IAP/IAM）＋cloudbuild `_BACKLOG_INTAKE_ENABLED` 4 箇所。意図的に非移植＝文書自動生成パイプライン（V2 は人が文書作成 funnel で作る）・BACKLOG_FIELD_* カスタムフィールド ID マップ・親子課題自動完了/受動チェーン・納期変更の完了時実行（U7 で後続）。tests 8件＝736緑・typecheck・build 緑。点火＝grant 046＋`_BACKLOG_INTAKE_ENABLED=true`＋`_BACKLOG_WEBHOOK_TOKEN_SECRET`（公開 ingress 2-4 が前提）。

| 2026-08-10 | Phase 16 | 16-3c 動的モーダル群（明細行・既存課題紐付け・納期変更依頼・grant/env 追加なし） | LegalBridge_AI_GCP | ✅ |

**16-3c 動的モーダル群**：V1 slackGateway の dispatch_action／views.update 系を移植し /法務依頼 を完成形へ。`line-items.ts`（純粋）＝明細定義4種（発注・許諾・納品・計算）を V1 と同一のラベル・選択肢で移植、最大5行・増減ボタン・構築/パース（空行スキップ）/formatLineItemsText（V1 同一書式）。modal.ts 動的化＝種別 select に dispatch_action、種別に応じ再構築（明細セクション＋private_metadata.li_count／検収書・計算書は候補セレクタ（__NEW__ 先頭・申請者の未完了依頼＝grant 044 の SELECT で抽出）＋対象契約番号ブロック／納期変更依頼は別フォーム＝候補＋自由キー＋新納期(+1日)＋理由・V1 同一検証）。handler 分岐＝①納期変更：作業課題は作らず承認用課題を起票→legal_requests(contract_type='deadline_change'・notes V1 同一構造 executed:false)＋台帳＋DM（実行は法務承認後＝V1 Phase 22.4 と同じ建付け・自動実行は B群 U7 で後続）②既存課題紐付け：新規課題なしで対象課題へフォーム内容（明細込み）をコメント記録＋台帳＋DM（V1 link-trigger-run の文書自動生成は後続）③検収書：契約番号の必須＋実在チェック・取引先自動補完（contract-check 注入時・複数契約は「複数 (n件)」表記）④計算書：非ブロッキング補完。dry-run は全分岐で台帳のみ。非移植＝署名URL/ポータルリンク（C群判断待ち）。**新規 grant・env・cloudbuild 変更なし＝点火条件は 16-3a と同一**。tests 14件＝728緑・typecheck・build 緑。

| 2026-08-10 | Phase 16 | 16-4 添付アップロード（依存フリー multipart＋案件添付・ATT 採番・新規 grant 不要） | LegalBridge_AI_GCP | ✅ |

**16-4 添付アップロード**：V1 worker の案件添付／課題番号添付を V2 の案件中心モデルへ寄せて移植。`multipart.ts`＝multipart/form-data の依存フリーパーサ（multer 非導入・boundary 抽出・エスケープ引用符・バイナリ耐性・express.raw で受けた Buffer を解釈）。DriveStorage に optional `uploadFile`（任意 MIME 生ファイル・Google/Memory 実装・既存スタブ非破壊）。attachments-repository（Pg/Memory）＝ATT-YYYY-NNNNN 採番（V1 と同じ document_sequences kind='attachment' 共有＝番号帯連続）→documents INSERT（is_primary=FALSE・lifecycle_status='final'・form_data に元ファイル名/mime/size/uploaded_via）→matters.updated_at touch ベストエフォート。**必要 grant は既存 006 のみ＝新規 grant 不要**。routes＝`POST /matters/:id/attachments`（admin/legal・種別3種 V1 同一・不明→reference・30MB・originalName 優先＝文字化け対策・Drive 名「課題番号(なければ案件コード/M{id})_アカウント_元名」・Drive 失敗502・42501→503・Backlog 気づきコメントは backlog-comment 点火時のみ非致命）。app guarded 定型（ATTACHMENT_UPLOAD_ENABLED×scope `attachments`×storage.uploadFile 必須）＋verify/cloudbuild 結線（scope は snippets の直後）。クライアント＝案件詳細・関連文書セクションに資料アップロードパネル（種別＋複数ファイル＋事前サイズチェック→登録後は関連文書一覧に ATT 行が自動で載る）。非移植＝課題番号ベースの公開ページ（portal 廃止判断待ち）・document_files 二重登録。tests 20件（パーサ7＋routes13）＝714緑・typecheck・build 緑。点火＝Drive ストレージ構成＋`_ATTACHMENT_UPLOAD_ENABLED=true`＋WRITE_SCOPES 追記のみ。

| 2026-08-10 | Phase 16 | 16-1 スニペットのサーバ共有化（text_snippets 共有・guarded・grant 045・旧ローカル移行導線） | LegalBridge_AI_GCP | ✅ |

**16-1 スニペット共有化**：Phase 6 の localStorage 完結（U17 退化）を解消し V1 `text_snippets`（0151）を全社共有で読み書き。repository（Pg/Memory・カテゴリ special_terms/work_item/other・一覧は有効行のみ category,sort_order,id 順・**42P01 空縮退＝V1 同様**・upsert-by-id・論理削除 is_active=false＝DELETE grant 不要）＋routes（GET 全ロール {snippets,writeEnabled}／POST・deactivate は guarded admin/legal・42501→SNIPPETS_FORBIDDEN_DB 503・未存在404）＋app.ts guarded-write 定型一式（SNIPPETS_WRITE_ENABLED×scope `snippets`×safe-write bypass・capability 公開）＋grant 045（SELECT/INSERT/UPDATE＋seq・token GRANT_PRODUCTION_SNIPPETS）＋verify/cloudbuild 結線（WRITE_SCOPES は contract-master の直後）。クライアント TextSnippets.tsx を全面書換：共有一覧（カテゴリ別グループ・検索・コピーは全ロール）＋編集フォーム（canEdit のみ・分類/タイトル/表示順/本文）＋**旧ローカル下書きの「共有へ移行」（サーバ保存→ローカル削除）／削除導線**。ナビ文言「この端末に保存」→「全社共有」。tests 13件＝694緑・typecheck・build 緑。点火：grant 045＋`_SNIPPETS_WRITE_ENABLED=true`＋WRITE_SCOPES 追記（`phase16-cutover-gaps.md`）。
...
| 2026-08-10 | Phase 16 | 16-3a Slack 法務依頼インテーク受信口（署名検証 fail-closed・/法務依頼 モーダル・Backlog起票＋legal_requests/issue_workflows・grant 044） | LegalBridge_AI_GCP | ✅ |

**16-3a Slack インテーク**（載せ替え必須 U1+U20 の第1スライス）：V1 の Cloud Run 世代 slackGateway を正として移植（GAS 足場は移植しない）。`/internal/slack/{commands,interactivity}`＝署名検証（v0 HMAC・リプレイ窓300s・**fail-closed**＝V1 の fail-open を是正）でユーザー認証をバイパス。`/法務依頼`→views.open（静的モーダル：種別9種/件名/納期+7d/詳細/相手方3項目任意）→提出で Backlog 起票（backlog-web-api に createIssue 追加・種別名解決）→legal_requests＋issue_workflows('文書生成依頼') INSERT（0103 トリガで matters 自動生成＝V2 案件モデルに自動接続）→隔離台帳 lb_v2_slack_intake_ledger→完了ビュー（実課題キー）。通知（依頼者DM＋部署ch）は応答後ベストエフォート。Backlog 非 live は dry-run＝台帳のみ。grant 044（台帳新設＋legal_requests/issue_workflows INSERT・guard で matters/matter_issues INSERT 事前検査）。config SLACK_INTAKE_ENABLED/SLACK_SIGNING_SECRET・verify ゲート・cloudbuild（_SLACK_SIGNING_SECRET_NAME→Secret 注入）全結線。tests 15件追加＝664緑。点火は公開 ingress 決定が前提（`phase16-cutover-gaps.md`）。残 16-3b：明細行・紐付け・納期変更・/法務検索（16-2 依存）。

| 2026-08-10 | 監査/S-F | UX 一括是正（P1-6〜P1-14：未有効化の型統一・内部識別子除去・偽アフォーダンス解消・ナビ3分割・派生元ピッカー） | LegalBridge_AI_GCP | ✅ |

**S-F UX 一括是正**（監査 P1-6〜P1-14・UI/UX 合理性と視認性）：①P1-6 ExcelBatch に FeatureLockedNote（無効時もボタンが黙って消えず「出力は可・記録のみ未有効」を明示）。②P1-7 WorkDetail の private Degraded() を FeatureLockedNote に統一（Q2 完遂）。③P1-8 内部識別子の end-user 露出を平易化：DataQuality「GRANT」／WorkDetail parent_work_id・work_relations／Requests「3-2b」／ナビ説明の GRANT。④P1-9 `.registry-table.static-rows` 新設で承認ルート・契約マスタの偽クリック行（cursor:pointer/hover 継承）を解消。⑤P1-10 WorkDetail の虚偽「読み取り専用」表記を修正。⑥P1-11 一括無効化の確認段に影響説明（単票と同内容）。⑦P1-12 空状態を EmptyState に統一（hub-note 第3流派を廃止）・`registry-state.neutral` 新設（無効フラグ/カテゴリに voided 赤・complete 緑を流用しない）・契約マスタの閲覧状態にバッジ（executed=緑/終了系=赤/進行中=橙）・キャンセル語彙を統一（やめる→キャンセル）。⑧P1-13 旧「マスタ・設定」12項目をマスタ(台帳/契約マスタ/契約取込/担当者/受信取込)／データ整備(品質/名寄せ×2)／設定・運用(設定/承認ルート/管理/ガイド)へ3分割＋契約マスタ→契約取込の surface-xref 配線（F5 対処）。⑨P1-14 派生元の生ID入力を検索ピッカー（デバウンス・自己除外・選び直し）に置換＋ハードコード色の inline style 除去。契約マスタの inline style も cm-* クラスへ。P1 残は P1-3/P1-4（vendor 整合＝別スライス）と P1-16（TZ 運用ルール）/P1-17（10-6 配線残）/P1-18（許容）。テスト649件緑・typecheck・build 緑。

| 2026-08-10 | 監査/S-E | 案件削除の整合（影響列挙を V1 全子表に拡張＋lb_v2 スレッド孤児防止・grant 042） | LegalBridge_AI_GCP | ✅ |

**S-E 案件削除整合**（監査 P0-10）：削除確認画面の影響列挙が V1 の子表 matter_slack_threads（FK CASCADE＝連鎖削除される）と document_files（SET NULL＝解除）を欠き過少申告だった。IMPACTS に両表＋lb_v2_matter_slack_threads を追加（UI は汎用レンダリングのため行追加のみで反映・件数不明は「不明（表示権限なし）」表示）。削除トランザクション内の件数取得は表ごとに SAVEPOINT で 42501/42P01/42703 を degrade（削除自体は止めない）。**V2 独自の lb_v2_matter_slack_threads は FK を持たず孤児化→UNIQUE(matter_id) が matter_id 再利用時のスレッド作成を恒久ブロック**するため、削除トランザクション内で明示 DELETE（grant 042 未適用なら SAVEPOINT で skip・結果 v2SlackThreads=null）。**grant 042**＝①preview 件数用 SELECT（matter_slack_threads/document_files・存在するものだけ動的付与）②lb_v2_matter_slack_threads への DELETE（ポインタ表であり監査台帳ではない・V1 同等表は FK CASCADE を持つ）③既存孤児の一掃 DELETE（冪等）＋事後検証。MatterDeleteResult は cascaded.slackThreads／unlinked.files／v2SlackThreads を追加。テスト649件緑・typecheck・build 緑。**監査 P0 はコード側全11件完了**（本番適用待ち：039/040/041/042＋reissue 点火手順）。残：S-F UX 一括是正（P1-6〜P1-14）。

| 2026-08-10 | 監査/S-D | 再発行を V1 準拠の repoint 方式へ再設計（実績は void せず新版へ付け替え＝残高不変・grant 041） | LegalBridge_AI_GCP | ✅ |

**S-D 再発行 repoint 再設計**（監査 P0-1・最重大の是正）：V2 旧実装は再発行時に旧版の condition_events を一括 void しており、消化済み発注の再発行で残高が全額復元（二重払い露出）＋V1 横断検索から条件明細が消える。V1 Phase E-1（server.ts:16969「void ではなく付け替えなので残高は不変」）に合わせ、**系列旧版の有効実績（voided_at IS NULL）の document_id を新版へ UPDATE**（voided_at/void_reason には触れない）に変更。condition_line_id は旧版明細のまま＝残高ビュー（condition_line_status_v）は明細基準なので消化維持・V1 の「非void 実績を持つ明細を表示」フィルタも旧版明細を出し続ける。V1 の reissueCarryover（明細の付替え）は「新版に明細をゼロから作り直す」場合の一意対応処理で、V2 は新版に明細を作らないため構造的に対象が生じない（フェイルセーフ設計とも整合）。残高復元の導線は「新版の void」に一本化（付け替えた実績ごと取消される）。demote に updated_at=now() 追加（V1 同様・grant 039 の範囲）。台帳 lb_v2_document_reissue_ledger は canceled_events→carried_events へ改名（意味の変更を列名に反映）。**grant 041**＝condition_events.document_id の列 UPDATE＋台帳列改名（冪等・事後検証 DO 付き）。preflight 041 は「旧方式で void された実績」を列挙＝個別リカバリ判断の材料。API/UI も canceledEvents→carriedEvents・文言を「新版へ引き継ぎ（残高不変）」に統一。**点火順序厳守**：S-D コードデプロイ→041 適用→scope 復帰（audit doc に手順）。テスト649件緑（repoint 検証＝voidedAt null のまま documentId が新版に付く）・typecheck・build 緑。残：S-E 案件削除整合→S-F UX 一括。

| 2026-08-10 | 監査/S-C | 文書ライフサイクル整合（finalize/reissue の業務列刻印＋040 バックフィル＋registry/検収フィルタ整合） | LegalBridge_AI_GCP | ✅ |

**S-C 文書ライフサイクル整合**：①P0-3＝finalize の INSERT に `record_type`（新設 `document-business-columns.ts` の純関数 `deriveRecordType`・V1 server.ts:9676 の分岐準拠：pub_master→master_contract／pub_→publication_condition／license・royalty→license_condition／PO・検収→individual_contract／他→master_contract）＋`contract_status='executed'`（V1 既定。CloudSign 送信フロー導入時に awaiting_signature 追加予定）＋`contract_title`（TITLE_KEYS）＋`vendor_id`（PARTY_NAME_KEYS の相手先名→vendor_name/trade_name/pen_name 解決・未解決でも作成継続）。②P0-4＝reissue の INSERT を INSERT..SELECT 化し旧版から業務列23列（vendor/契約種別/表題/有効期間/台帳参照/条件番号系…）を丸ごと継承＋`COALESCE(contract_status,'executed')`＝V1 の tg_doc_autolink_contract が空 contracts 行を捏造しない。③P1-5＝is_primary 降格を `template_type` 単位に限定（V1 の正本選定と同義・同系列の検収書を巻き込まない）。④P1-1＝registry「有効」フィルタは voided/reissued/superseded 除外（V1 横断検索と同義・UI ラベルも明示）。⑤P1-2＝検収ワークリストは現行版＋正本の発注のみ・検収書 EXISTS は voided 除外。⑥040 バックフィル SQL＝既存 V2 作成文書へ contract_status='executed' を一回限り是正（**V2 判定 = `template_version_id IS NOT NULL`**：V1 は 0002 で列だけ追加しコードが一切書かないため確定判定・V1 の NULL 行は触らない・冪等）＋preflight（種別別件数・サンプル・対象外件数）。INSERT はテーブルレベル grant 006 の範囲内＝新 GRANT 不要。テスト649件緑（record_type 分岐・firstTextValue・P1-5 降格スコープ）・typecheck・build 緑。残：S-D 再発行 repoint（点火禁止継続）→S-E 案件削除整合。

...
| # | 機能 | 由来 | 判断 | 粒度 |
|---|---|---|---|---|
| 16-1 | スニペットのサーバ共有化（text_snippets 表・カテゴリ・全社共有） | U17 | ✅ 実装済（guarded・grant 045・旧ローカル下書きの移行導線付き・`phase16-cutover-gaps.md`） | 小 |
| 16-2 | 契約チェック API（用途マスタ×スコープ判定） | U2 | ✅ 実装済（読取専用・判定文字列 V1 同一・用途マスタは TS 定数化＝grant 不要・`phase16-cutover-gaps.md`） | 中 |
| 16-3 | **Slack スラッシュコマンド/インタラクティビティ受信口＋法務依頼インテーク** | U1+U20 | ✅ 16-3a＋16-3b＋16-3c 実装済（受信口＋/法務依頼＝明細行・紐付け・納期変更込み＋/法務検索・grant 044）。残＝署名URL/ポータルリンク（C群判断待ち）と納期変更の自動実行（B群 U7） | 大 |

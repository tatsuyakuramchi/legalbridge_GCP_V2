...
## 26. Slice 1 / 2 の実施結果（2026-08-17）

### §10 差分表の再確認

計画の差分表は調査時点の V2 を写したもので、着手時には transport の大半が既に
V1 互換になっていた。実測した現状は以下。

| 項目 | 計画の記載 | 着手時の実装 | 対応 |
|---|---|---|---|
| token を form-urlencoded で送る | 未対応 | 対応済み | 変更なし |
| `expires_in` を尊重した cache | 未対応 | 対応済み（30秒前倒し失効） | 変更なし |
| 401 で1回だけ再取得 | 未対応 | 対応済み | 変更なし |
| create document を form-urlencoded | 未対応 | 対応済み | 変更なし |
| multipart field `uploadfile` | `files` のまま | 対応済み | 変更なし |
| participant を form-urlencoded | 未対応 | 対応済み | 変更なし |
| participant の order / language_code | 未対応 | 未対応 | **任意項目として追加** |
| upstream response body の保持 | 未対応 | 未対応 | **実装** |

したがって Slice 1 の実質的な残件は「upstream error を捨てないこと」と
participant の任意項目2つだった。

### 実装

- `cloudsign-upstream-error.ts`（新規・純関数）
  - `parseUpstreamBody` … JSON / 非 JSON（HTML エラーページ）の双方から code・message を取る
  - `classifyCloudSignFailure` … §13.2 の6分類へ振り分ける
  - `redactSecrets` … client_id / access_token / Bearer をログ・UI 双方から落とす
  - `toClientError` / `toLogLine` … UI 向け正規化とログ1行
- `cloudsign-api-adapter.ts`
  - 失敗時は `json()` ではなく `text()` で本文を読む（JSON とは限らないため、
    従来は本文ごと失われていた）。`/token` の失敗も同様に保持する
  - `CloudSignError.detail` に status / method / path / upstreamCode / upstreamMessage /
    kind / retryable を載せる
  - participant に `order` / `language_code` を任意で送る
- `cloudsign-routes.ts` … `detail` があれば原因別コードで返す。retryable は 503、
  それ以外は 502。生本文は返さない
- `DocumentIntegrations.tsx` … 分類メッセージ＋CloudSign の一文＋再試行可否を表示

### 「アドレス」問題の切り分け（§11 原則2 の実装）

`許可されていないアドレスです` は宛先と送信元 IP の双方で使われる文言のため、
**IP を明示する語（ip / 接続元 / 送信元 / allowlist）が無い限り IP 制限と判定しない**。
「アドレス」しか手掛かりが無い場合は宛先の問題として扱い、診断順序 5a → 5c を守る。

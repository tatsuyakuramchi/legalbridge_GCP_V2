...
- **本番の現行版は v12**（リポジトリ参照の 0120 より2版先: v11=支払日グループ化＋明細別変更履歴
  PR #478、v12=¥二重表示修正 0153）。詳細表は7列（今回数量/歩留率/納品日/**支払日**/支払対価）・
  合計行 `colspan="6"`。059 はこの v12 構造に合わせてある（初版は 0120 前提で、ガードが
  「歩留率の列見出しが1箇所ではない」と正しく中断した→本番ダンプで実構造を確認して再作成）。
- 全文差し替えではなく**現行版への文字列置換**で新版を作る＝本番側に未知の改訂があっても温存。
  置換対象（歩留率の見出し width:9%・acceptance_ratio セル・成果物列 width:38%・
  単票行の「—」対・合計行 colspan="6" ×3種）が想定どおり見つからないときは何もせず中断する。
  支払日グループ化表示（useGroupedInspection）はもともと歩留率なしのため触らない。
- `acceptance_ratio` はデータ・計算（単価×数量×歩留率）では温存し、**表示だけ**落とす。
  フォーム側も歩留率入力は出さない（引用時は既定 1.0）。明細テーブルには v12 の
  行別支払日（paid_date・未入力は支払期日にフォールバック）の入力列を追加済み。
...
適用後の確認: 最後の SELECT で `yield_visible_pos = 0`・`colspan5_count >= 3`（新版は v13）。

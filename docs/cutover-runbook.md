...
| # | 内容 | 状態 |
|---|---|---|
| 1-1 | **Phase 16-3：Slack インテーク** | ✅ 16-3a 実装済（受信口＋/法務依頼＋起票/記録/通知・grant 044）。点火は `phase16-cutover-gaps.md`。16-3b（明細行/紐付け/納期変更/法務検索）残 |
| 1-2 | Phase 16-1：スニペットのサーバ共有化 | 未着手（小） |
| 1-3 | Phase 16-2：契約チェック API（用途×スコープ判定） | 未着手（中） |

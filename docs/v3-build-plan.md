...
| # | 成果物 | 状態 |
|---|---|---|
| 1-1 | `infra/v3/001_schema.sql` — 28テーブル | **完了**（PostgreSQL 16 で実行検証・冪等）※本番は 15.18。PG16固有構文・拡張機能ともに不使用 |
...
モックの構成どおり3段。案件が制御レイヤー、他は参照。

| 段 | 画面 |
|---|---|
| 入口 | ホーム（条件から導出した数字と期限）／案件（フロー種別で中身が変わる） |
| 横断で見る | 条件／作品（権利包絡）／取引先・担当／文書／お金 |
| 監視・運用 | フロー監視（取適法の遵守一覧・作品ごとの権利上限）／運用（データ品質・期限・監査・設定） |
...
## Step 1 の実行結果（本番データ・2026-09-08）

Cloud SQL（PostgreSQL 15.18）へ適用済み。全12エンティティと金額が一致。
...
金額（JPY）: 116,601,202 = 116,601,202。

### 実データで見つかった V1 側の問題

移行スクリプトはこれらを落とさず取り込み、`data_quality_issues` に残す。
切替前に0にする対象は `high` の304件。

| ルール | 件数 | 内容 |
|---|---|---|
| `MIGRATION_AGREEMENT_NO_PARTY` | 166 | 相手先未入力の契約。うち34件は `ATT-` の添付ファイル |
| `MIGRATION_CONDITION_NO_PARTY` | 87 | 相手先未入力の条件・¥12,470,828 |
| `MIGRATION_PAYMENT_NO_PARTY` | 25 | 相手先未入力の支払 |
| `PAYMENT_UNALLOCATED` | 25 | 条件に割り当てのない支払（同じ25件） |
| `AGREEMENT_TERM_INVERTED` | 1 | `contracts.id=2792` の開始日が `2226-04-21` |
| `CONDITION_PRICING_RECLASSIFIED` | 101 | `calc_method` の宣言と実データの食い違い（376件中27%） |
| `CONDITION_NO_WORK` | 71 | 作品に紐づかないライセンス条件 |
| `DOCUMENT_NO_SOURCE` | 50 | 発行済みだがテンプレも保管先も無い文書 |
| `WORK_SOURCE_IP_MERGED` | 12 | `source_ips` 12件は `works` の二重登録だったため統合 |
| `WORK_PART_NO_RENUMBERED` | 2 | `material_no` が重複・欠落していた作品 |

### 移行中に直した構造上の問題

- `works` と `source_ips` の採番空間が重なる（12件すべて衝突）
- 連番の採番（`work_parts` / `condition_schedules` / `document_conditions`）が
  重複を作る。並行稼働中に移行元の行が消えると再実行で破綻する点も含めて修正
- 契約期間の逆転（`CHECK` に当たって380件が巻き戻る）
- 相手先未入力の行が全部落ち、¥23,131,635 を黙って過少計上していた
- 価格方式の宣言と実データの食い違い
- ビューへの書込権限。`ALTER DEFAULT PRIVILEGES ... ON TABLES` はビューにも
  効くため、`002_views.sql` を流し直すと書込権限が付いた。さらに検証クエリの
  `LIKE 'v\_%'` が `standard_conforming_strings` の下では
  「v + バックスラッシュ + 任意1文字」を意味し、**このチェックは最初から
  素通りしていた**。既定を `SELECT` のみにし、判定を `pg_views` に変え、
  `003` の再実行で自己修復するようにした

いずれも「V3 の整合規則は緩めない。実データに合わせて解決し、記録を残す」
方針で処理した。
...
実行手順は **[docs/v3-deploy-runbook.md](./v3-deploy-runbook.md)** に切り出した。
...
- **取引先の整理**。2,552件は契約380件・条件376件に対して多く、重複や
  未使用の行が溜まっている可能性がある。移行は `src 2,552 = dst 2,552` で
  正確なので、V3 側に問題は無い。いま master を整理すると突き合わせの
  基準がずれ、移行が正しかったのか整理の結果なのか区別できなくなるため、
  切替後に V3 の画面から名寄せする。
- V3 の取引先一覧は `status` で絞っていない（`archived` も出る）。V1 が
  有効のみを表示しているなら件数が食い違って見える。突き合わせの妨げに
  なるようなら、既定を有効のみにして切替を付ける。

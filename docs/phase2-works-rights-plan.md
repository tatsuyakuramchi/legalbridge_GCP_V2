...
| **2-4b** | work_relationsグラフ編集（derived_from追加・系譜の第2真実源との整合） | 書込 | 007（INSERT済）流用・新規不要 | ✅（削除は要DELETE判断で保留） |

### 決定ポイント（着手時に確認）
- **製品(製品/products)**：DBにテーブルが存在しない（テンプレの自由入力のみ）。移植は**新規テーブル＝DDL**を要し、「共有DB無改変」原則に抵触。→ **決定：オプションA（既存エンティティで代替）を採用**。作品詳細に「製品」タブを追加し、派生作品（子works）＋構成素材から製品相当を**読取集約**（DDL・GRANT不要）。正式な製品モデル（SKU/発売日/版を持つ `products` テーブル＝オプションB）が必要になった時点でDBA承認のもと置換する。
